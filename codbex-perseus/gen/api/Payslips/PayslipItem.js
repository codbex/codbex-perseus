...
rs.service()
	...
	.resource("count/{PayslipId}")
		.get(function(ctx, request) {
			let PayslipId = parseInt(ctx.pathParameters.PayslipId);
			PayslipId = isNaN(PayslipId) ? ctx.pathParameters.PayslipId : PayslipId;
			http.sendResponseOk("" + dao.count(PayslipId));
		})
		.catch(function(ctx, error) {
            if (error.name === "ForbiddenError") {
                http.sendForbiddenRequest(error.message);
