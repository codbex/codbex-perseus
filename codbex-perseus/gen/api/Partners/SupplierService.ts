...
import { SupplierRepository, SupplierEntityOptions } from "../../dao/Partners/SupplierRepository";
import { ValidationError } from "../utils/ValidationError";
import { HttpUtils } from "../utils/HttpUtils";

const validationModules = await Extensions.loadExtensionModules("codbex-perseus-Partners-Supplier", ["validate"]);
...
class SupplierService {
    ...
    public create(entity: any) {
        try {
            ...
            response.setHeader("Content-Location", "/services/ts/codbex-perseus/gen/api/Partners/SupplierService.ts/" + entity.Id);
    ...
    private validateEntity(entity: any): void {
        ...
        if (entity.SWIFT?.length > 20) {
            throw new ValidationError(`The 'SWIFT' exceeds the maximum length of [20] characters`);
        }
        if (entity.Bank?.length > 100) {
            throw new ValidationError(`The 'Bank' exceeds the maximum length of [100] characters`);
        }
